...
There are 2 ways of writing a "dig" replacement for Windows.  The first is to completely construct the DNS query packet then sending it out the raw UDP socket interface.  The second is to use the built in Windows APIs for DNS.  The 2 are a Winsock API that is fairly full featured the second is a Powershell cmdlet which is less featured and is unable to make several kinds of DNS queries.  So, some of the Powershell scripts here assemble the DNS query packet manually and use the raw UDP socket available in Powershell to send the query.

The dns-proxy-test Powershell script uses the ISC windows dig command for one of it's tests.  That is optional.  The script was written for the purpose of discovering transparent DNS proxies.  The Comcast ISP that I use runs a transparent DNS proxy that caused interference when I setup a DNS server on a static IP block from Comcast.  At that time I was not aware that Comcast ran a transparent proxy.  After this possibility was suggested on the DNS mailing list I wrote the script to verify it.  It runs multiple tests that can identify if a transparent DNS proxy exists between the PC it is running on and the Internet.
